  ...
  private async processFileList(
    ...
    for (let i = 0; i < filePaths.length; i++) {
      ...
      try {
        ...
        if (chunks.length > 50) {
          console.warn(
            `[Context] ⚠️  File ${relativeFilePath} generated ${chunks.length} chunks (${Math.round(content.length / 1024)}KB)`,
          );
        } else if (content.length > 100000) {
          console.log(
            `📄 Large file ${relativeFilePath}: ${Math.round(content.length / 1024)}KB -> ${chunks.length} chunks`,
          );
        }
        ...
        console.warn(
          `[Context] ⚠️  Skipping file ${relativeFilePath}: ${error}`,
        );
      }
    }
