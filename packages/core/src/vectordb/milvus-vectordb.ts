...
export class MilvusVectorDatabase extends BaseVectorDatabase<MilvusConfig> {
  ...
  protected async resolveAddress(): Promise<string> {
    ...
    if (!finalConfig.address && finalConfig.token) {
      finalConfig.address = await ClusterManager.getAddressFromToken(
        finalConfig.token,
      );
    }
  ...
  protected async ensureLoaded(collectionName: string): Promise<void> {
    if (!this.client) {
      throw new Error(
        "MilvusClient is not initialized. Call ensureInitialized() first.",
      );
    }
    ...
    try {
      ...
      if (result.state !== LoadState.LoadStateLoaded) {
        console.log(
          `[MilvusDB] 🔄 Loading collection '${collectionName}' to memory...`,
        );
  ...
  protected async waitForIndexReady(
    ...
    while (Date.now() - startTime < maxWaitTime) {
      try {
        ...
        console.log(
          `[MilvusDB] 📊 Full response:`,
          JSON.stringify(indexBuildProgress),
        );
        ...
        if (
          ...
          if (
            ...
            indexBuildProgress.status.reason.includes(
              "index duplicates[indexName=]",
            )
          ) {
            console.log(
  ...
  protected async loadCollectionWithRetry(
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        console.log(
          `[MilvusDB] ✅ Collection '${collectionName}' loaded successfully!`,
        );
        return;
      } catch (error) {
        console.error(
        ...
        console.log(
          `[MilvusDB] ⏳ Retrying collection load in ${interval}ms...`,
        );
        await new Promise((resolve) => setTimeout(resolve, interval));
  ...
  async createCollection(
    ...
    const createCollectionParams = {
      collection_name: collectionName,
      description:
        description || `Claude Context collection: ${collectionName}`,
      fields: schema,
    };
  ...
  async dropCollection(collectionName: string): Promise<void> {
    ...
    if (!this.client) {
      throw new Error(
        "MilvusClient is not initialized after ensureInitialized().",
      );
    }
  ...
  async query(
    ...
    try {
      ...
      console.error(
        `[MilvusDB] ❌ Failed to query collection '${collectionName}':`,
        error,
      );
      throw error;
    }
  ...
  async hybridSearch(
    ...
    try {
      ...
      console.log(
        `[MilvusDB] 🔍 Preparing hybrid search for collection: ${collectionName}`,
      );
      ...
      console.log(
        ...
        JSON.stringify(rerank_strategy, null, 2),
      );
  ...
  async checkCollectionLimit(): Promise<boolean> {
    ...
    try {
      ...
      if (
        await this.client.hasCollection({ collection_name: collectionName })
      ) {
        await this.client.dropCollection({
          collection_name: collectionName,
        });
