...
export class SnapshotManager {
  ...
  private loadV2Format(snapshot: CodebaseSnapshotV2): void {
    ...
    for (const [codebasePath, info] of Object.entries(snapshot.codebases)) {
      ...
      if (info.status === "indexed") {
        ...
        console.warn(
          `[SNAPSHOT-DEBUG] Found interrupted indexing codebase: ${codebasePath} (${info.indexingPercentage || 0}%). Treating as not indexed.`,
        );
   ...
   * @deprecated Use setCodebaseIndexed() instead for v2 format support
   */
  public moveFromIndexingToIndexed(
    codebasePath: string,
    fileCount?: number,
  ): void {
  ...
  public removeCodebaseCompletely(codebasePath: string): void {
    ...
    console.log(
      `[SNAPSHOT-DEBUG] Completely removed codebase from snapshot: ${codebasePath}`,
    );
  }

  public loadCodebaseSnapshot(): void {
    console.log(
      "[SNAPSHOT-DEBUG] Loading codebase snapshot from:",
  ...
  public saveCodebaseSnapshot(): void {
    console.log(
      "[SNAPSHOT-DEBUG] Saving codebase snapshot to:",
      this.snapshotFilePath,
    );
    ...
    try {
      ...
      if (!fs.existsSync(snapshotDir)) {
        ...
        console.log(
          "[SNAPSHOT-DEBUG] Created snapshot directory:",
          snapshotDir,
        );
      }
